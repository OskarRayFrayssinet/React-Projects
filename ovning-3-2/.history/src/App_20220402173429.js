import './App.css';
import TheaterText from './components/TheaterText';
import KlickButtonOption from './components/KlickButtonOption';
import ListObjectInsult from './components/ListObjectInsult';

function App() {

  //Array with insults and plays
  let insultsArray = [
  ...
  function returnRandomInsult() {
    return insultsArray[(Math.floor(Math.random() * (insultsArray.length - 1)))];
  }

  return (
    <div className="App">
      <TheaterText insultObject={returnRandomInsult()} insults={insultsArray} />
      <KlickButtonOption buttonOptionType="openInsults" insults={insultsArray} />
      <KlickButtonOption buttonOptionType="generateRandomInsult" insults={insultsArray} />
      <KlickButtonOption buttonOptionType="buttonNotFound" insults={insultsArray} />
    </div>
  );
}
